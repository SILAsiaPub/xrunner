...
/* Description: find the ini section then process is with line handler
	...
	found = 0
	if lines(in) == 1 
		then call info 2 nameext(in) 'found! Getting section [' || section || '] processed by' dofunc
		else say 'XXX Did not detect file "' || in || '" lines returned' lines(in)
	do while lines(in) > 0 
		line = strip(linein(in))
		/* say line */
...
/* Description: find the ini section then process is with line handler
	...
	found = 0
	if lines(in) == 1 
		then do call info 2 nameext(in) 'found! Getting section [' || section || '] processed by' dofunc ; call beep note.i,250 end
		else say 'XXX Did not detect file "' || in || '" lines returned' lines(in)
	do while lines(in) > 0 
		line = strip(linein(in))
	...
	if lines(in) == 1 
		then 
			do 
				...
				call beep note.i,250 
			end
		else do say 'XXX Did not detect file "' || in || '" lines returned' lines(in) end
	do while lines(in) > 0 
		line = strip(linein(in))
...
xslt:
  ...
  b = infile(b)
  c = outfile(c,group || "-" || count || "-" || xname || ".xml") 
  if lines(a) == 0 then do fatal = "true"; say "Fatal: Missing XSLT file"; end
  if lines(b) == 0 then do fatal = "true"; say "Fatal: Missing input XML file"; end
  c = "-o:" || c
  call info 3 c
  call info 2 "java -jar" SAXON c b a d e f g h
  --if info3 == "on" then say c
...
/* Description: find the ini section then process is with line handler
	...
	out = 0
	comment = 'Auto generated temporary file. Created from' FILESPEC("n",in) 'extracting section ['section'] by process' dofunc
	call info 3 STREAM(outf,"C",'write')
	select
		/* when arg(5) == 1 then out = lineout(arg(2),'/''* auto generated temporary file from' FILESPEC("n",arg(1)) 'from section' arg(3)'*''/ ',arg(5))  */
		when 'writexslt' == dofunc then
			do
